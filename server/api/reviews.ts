        ...
        if (googleRes.ok) {
          ...
          if (googleData.result && googleData.result.reviews) {
            ...
            for (const r of googleData.result.reviews) {
              if (r.rating >= 4 && r.text && r.text.length > 20) {
                reviews.push({
                  id: `google-${r.time}`,
                  source: 'Google',
                  author: r.author_name,
                  rating: Math.floor(r.rating), // ensure integer
            ...
            for (const r of yelpData.reviews) {
              if (r.rating >= 4 && r.text && r.text.length > 20) {
                reviews.push({
                  ...
                  date: r.time_created.split(' ')[0], // simple date formatting
                  text: r.text,
                  url: r.url,
                  timestamp: new Date(r.time_created).getTime() / 1000
                })
              }
            }
          }
