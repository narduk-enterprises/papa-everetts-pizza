...
interface FormHandlerOptions<T extends Record<string, any>> {
...
export function useFormHandler<T extends Record<string, any>>(options: FormHandlerOptions<T>) {
  const {
    schema,
    defaults,
    endpoint,
  ...
  async function submit() {
    ...
    try {
      ...
      const message = err?.data?.message || err?.message || errorMessage
      toast.add({
        title: 'Error',
        description: message,
        color: 'error',
