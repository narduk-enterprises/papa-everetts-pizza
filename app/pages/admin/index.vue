          ...
          <div class="p-6 sm:p-8 space-y-8">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
              ...
              <UFormField label="Sort Order" class="md:col-span-2 lg:col-span-2">
                <UInput v-model="entry.draft.sortOrder as any" type="number" size="lg" class="w-full" />
              </UFormField>
              ...
              <UFormField label="Image" class="md:col-span-12">
                <div class="flex items-center gap-4">
                  ...
                  <div class="flex items-center gap-2">
                    <UButton color="neutral" variant="soft" icon="i-lucide-upload" @click="() => ($refs[`file-${entry.item.id}`] as any)?.[0]?.click?.() || ($refs[`file-${entry.item.id}`] as any)?.click?.()">
                      {{ entry.draft.imageUrl ? 'Change Image' : 'Upload Image' }}
                    </UButton>
                    <!-- Hidden file input for logic -->
                    <!-- eslint-disable-next-line atx/no-native-input -->
                    <input
                      :ref="`file-${entry.item.id}`"
                      type="file"
                      class="hidden"
                      accept="image/*"
                      @change="e => onUploadImage(e, entry.draft)"
                    />
        ...
        <div class="rounded-2xl border-2 border-dashed border-primary-200 bg-primary-50/30 p-6 sm:p-8 shadow-sm">
          ...
          <div class="grid grid-cols-1 md:grid-cols-12 gap-6 items-start mt-6">
            ...
            <UFormField label="Sort Order" class="md:col-span-2">
              <UInput v-model="newItem.sortOrder as any" type="number" size="lg" class="w-full" />
            </UFormField>
            ...
            <UFormField label="Image" class="md:col-span-12">
              <div class="flex items-center gap-4">
                ...
                <div class="flex items-center gap-2">
                  <UButton color="neutral" variant="soft" icon="i-lucide-upload" @click="() => ($refs.newFile as any)?.click?.()">
