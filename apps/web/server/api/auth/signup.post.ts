...
export default defineEventHandler(async (event) => {
  ...
  const isFirstUser = (await getUserCount(event)) === 0
  ...
  if (!isFirstUser) {
    ...
    const auth = await getAuthSession(event, sessionId)
  ...
  const user = await createUser(event, email, password, name, isFirstUser)
