...
<template>
  <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-6 sm:space-y-8">
    <div class="space-y-2 sm:space-y-3">
      ...
      <p class="text-[var(--color-pizza-muted)] max-w-3xl text-sm sm:text-base">
    ...
    <div class="grid sm:grid-cols-[1.2fr_1fr] gap-4 sm:gap-6 items-stretch">
      ...
      <div class="warm-card p-5 sm:p-6 space-y-3">
        <p class="uppercase text-xs tracking-[0.2em] text-[var(--color-pizza-primary)] font-semibold">Pricing Basis</p>
        <p class="text-lg font-semibold">Build Your Own Masterpiece</p>
        <p class="text-[var(--color-pizza-muted)] text-sm">
        ...
        <p class="text-[var(--color-pizza-muted)] text-sm">For exact order totals <ULink :to="restaurantInfo.phoneHref" class="text-[var(--color-pizza-primary)] hover:text-[var(--color-pizza-primary-dark)] transition-colors underline decoration-[var(--color-pizza-primary)]/40 underline-offset-4 font-medium">call (641) 357-4040</ULink>.</p>
      </div>
    </div>

    <div v-if="pending" class="text-[var(--color-pizza-muted)]">Loading builder pricing...</div>
    <div v-else-if="error" class="text-[var(--color-pizza-primary)]">Unable to load current pricing right now.</div>
