...
function getOrderedPriceKeys(prices: MenuPrices) {
  return Object.keys(prices || {})
    ...
    .sort((a, b) => {
      const aIndex = menuSizeOrder.indexOf(a as (typeof menuSizeOrder)[number])
      const bIndex = menuSizeOrder.indexOf(b as (typeof menuSizeOrder)[number])
...
<template>
  <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="mb-8">
      ...
      <p class="text-[var(--color-pizza-muted)] mt-3 max-w-3xl">All prices reflect our current menu data. Tap any menu scan image to view full-size.</p>
    </div>

    <!-- Link to Paper Menu -->
    <div class="mb-10 p-6 sm:p-8 rounded-2xl bg-linear-to-br from-[var(--color-pizza-primary)]/5 to-transparent border border-[var(--color-pizza-primary)]/10 flex flex-col sm:flex-row items-center justify-between gap-6">
      <div>
        <h2 class="font-display text-2xl text-[var(--color-pizza-primary)] mb-1">Looking for the Original Paper Menu?</h2>
        <p class="text-[var(--color-pizza-muted)] text-sm max-w-lg">View high-resolution scans of the physical menu we hand out in the restaurant, complete with pinch-to-zoom support.</p>
      </div>
      <UButton
        to="/paper-menu"
        color="primary"
    ...
    <div v-if="pending" class="text-[var(--color-pizza-muted)]">Loading menu...</div>
    <div v-else-if="error" class="text-[var(--color-pizza-primary)]">Unable to load menu right now.</div>

    <div v-else class="space-y-10">
      <!-- ═══════════════════════════════════ Pizza Builder (integrated) ═══════════════════════════════════ -->
      <section id="pizza-builder" class="warm-card p-0 overflow-hidden border-2 border-[var(--color-pizza-primary)]/20">
        <UCollapsible class="w-full">
          <div role="button" tabindex="0" class="w-full text-left bg-linear-to-r from-[var(--color-pizza-primary)]/5 to-transparent p-6 sm:p-8 hover:bg-[var(--color-pizza-primary)]/5 transition-colors group flex items-center justify-between cursor-pointer outline-none">
            <div class="flex flex-col sm:flex-row sm:items-center gap-4">
              <div class="w-12 h-12 rounded-full bg-[var(--color-pizza-primary)]/10 flex items-center justify-center shrink-0">
                <UIcon name="i-lucide-chef-hat" class="size-6 text-[var(--color-pizza-primary)]" />
              </div>
              <div>
                <h2 class="font-display text-3xl font-normal text-[var(--color-pizza-text)]">Build Your Own Masterpiece</h2>
                <p class="text-[var(--color-pizza-muted)] mt-1 font-normal text-base text-left">Choose your size, add toppings, and see your estimated price live — including 7% local tax.</p>
              </div>
            </div>
            <UIcon name="i-lucide-chevron-down" class="size-6 text-[var(--color-pizza-primary)] transform transition-transform duration-200 group-data-[state=open]:rotate-180" />
          </div>

          <template #content>
            <div class="p-6 sm:p-8 border-t border-[var(--color-pizza-border)]">
      ...
      <section v-for="section in visibleCategories" :key="section.category" class="space-y-4">
        <!-- Category header -->
        <div class="border-b-2 border-[var(--color-pizza-primary)]/15 pb-3">
          <h2 class="font-display text-3xl">{{ section.category }}</h2>
          <p v-if="secondPizzaCategories.includes(section.category)" class="text-[var(--color-pizza-muted)] text-sm mt-1">
        ...
        <div class="grid md:grid-cols-2 gap-3">
          ...
          >
            ...
            <div v-if="item.imageUrl" class="shrink-0 w-full sm:w-28 sm:h-28 aspect-video sm:aspect-square overflow-hidden rounded-lg shadow-sm border border-[var(--color-pizza-border)]/50 group-hover:border-[var(--color-pizza-primary)]/30 transition-colors cursor-zoom-in" @click="openLightbox(item.imageUrl!, item.name)">
            ...
            <div class="flex-1 min-w-0 flex flex-col w-full">
              <div class="flex items-start justify-between gap-3">
                <div class="flex-1 min-w-0">
                  <p class="font-semibold text-lg transition-colors group-hover:text-[var(--color-pizza-primary)]">{{ item.name }}</p>
                  <p v-show="item.description" class="text-[var(--color-pizza-muted)] mt-1 text-sm leading-relaxed">{{ item.description || '' }}</p>
                </div>
                <div v-if="hasVisiblePrices(item) && priceEntries(item).length === 1" class="text-lg font-medium whitespace-nowrap text-[var(--color-pizza-text)] mt-0.5">
            ...
            <div v-if="hasVisiblePrices(item) && priceEntries(item).length > 1" class="mt-3 flex flex-wrap gap-2">
              ...
              >
                <span class="text-[var(--color-pizza-muted)] font-normal">{{ entry.label }}:</span>
            ...
            <div v-else-if="!hasVisiblePrices(item)" class="mt-3 text-sm font-medium">
              <ULink :to="restaurantInfo.phoneHref" class="text-[var(--color-pizza-primary)] hover:text-[var(--color-pizza-primary-dark)] transition-colors underline decoration-[var(--color-pizza-primary)]/40 underline-offset-4">
