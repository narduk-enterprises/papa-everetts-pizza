...
async function submitAuth(_event: FormSubmitEvent<AuthState>) {
  ...
  try {
    ...
    const msg = (error as { data?: { message?: string } })?.data?.message ?? 'Unable to authenticate.'
    toast.add({ title: 'Auth Error', description: msg, color: 'error', icon: 'i-lucide-alert-circle' })
  } finally {
...
async function onSaveItem(itemId: number) {
  ...
  try {
    ...
    toast.add({ title: 'Unable to save item', description: e?.data?.message ?? e?.message ?? 'Please review the fields and try again.', color: 'error', icon: 'i-lucide-alert-circle' })
  }
}
...
async function onDeleteItem(itemId: number) {
  try {
    ...
    toast.add({ title: 'Unable to deactivate item', description: msg, color: 'error', icon: 'i-lucide-alert-circle' })
  }
}
...
async function onAddItem() {
  try {
    ...
    const e = error as { data?: { message?: string }; message?: string }
    toast.add({ title: 'Unable to add item', description: e?.data?.message ?? e?.message ?? 'Please review the fields and try again.', color: 'error', icon: 'i-lucide-alert-circle' })
  }
}
...
interface ItemDraftWithImage { imageUrl?: string | null; [key: string]: unknown }
async function onUploadImage(event: Event, draft: ItemDraftWithImage) {
  ...
  try {
    ...
    const msg = (error as { data?: { message?: string } })?.data?.message ?? 'Please try again.'
    toast.add({ title: 'Upload failed', description: msg, color: 'error', icon: 'i-lucide-alert-circle' })
  } finally {
    target.value = ''
  }
}
...
async function onAddCategory() {
  ...
  try {
    ...
    const msg = (error as { data?: { message?: string } })?.data?.message ?? 'Unable to add category.'
...
async function saveEditCategory(catId: number) {
  ...
  try {
    ...
    const msg = (error as { data?: { message?: string } })?.data?.message ?? 'Unable to rename category.'
    toast.add({ title: 'Error', description: msg, color: 'error', icon: 'i-lucide-alert-circle' })
  }
}
...
async function onDeleteCategory(catId: number) {
  try {
    ...
    const msg = (error as { data?: { message?: string } })?.data?.message ?? 'Unable to delete category.'
    toast.add({ title: 'Cannot Delete', description: msg, color: 'error', icon: 'i-lucide-alert-circle' })
  }
}
</script>
...
<template>
  <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10" style="font-family: system-ui, -apple-system, sans-serif;">
    ...
    <!-- Non-admin notice -->
    <div v-else-if="!user?.isAdmin" class="mt-8 rounded-xl border border-pizza-border p-6 bg-elevated">
    ...
    <div v-else class="mt-8 grid xl:grid-cols-[250px_1fr] gap-6">
      ...
      <aside class="space-y-4">
        <div class="rounded-xl border border-pizza-border bg-pizza-surface p-4 shadow-sm xl:sticky xl:top-24">
          <div class="flex items-center justify-between mb-3">
            <p class="text-xs uppercase tracking-wider text-pizza-muted font-semibold">Categories</p>
        ...
        <div v-if="showCategoryPanel" class="rounded-xl border border-pizza-border bg-pizza-surface p-4 shadow-sm">
          <p class="text-xs uppercase tracking-wider text-pizza-muted font-semibold mb-3">Manage Categories</p>
      ...
      <div class="space-y-6 min-w-0">
        ...
        <div class="rounded-xl border border-pizza-border bg-pizza-surface p-5 shadow-sm">
          <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
            <div>
              <h2 class="text-2xl font-bold tracking-tight text-pizza-text">{{ activeCategory }}</h2>
        ...
        <!-- Empty state -->
        <div v-else-if="!currentItems.length" class="rounded-xl border border-dashed border-pizza-border p-8 text-center">
        ...
        <article
          ...
          :class="entry.draft.isActive ? 'border-pizza-border' : 'border-primary-200 bg-primary-50/20'"
        >
          <!-- Item header bar -->
          <div class="flex items-center justify-between px-6 sm:px-8 py-5 border-b border-pizza-border bg-elevated">
            <div class="flex items-center gap-4">
              <span class="font-display text-xl font-bold text-pizza-text tracking-tight leading-none">{{ entry.draft.name || 'Untitled' }}</span>
              <span
          ...
          <div class="p-6 sm:p-8 space-y-8">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
              ...
              <UFormField label="Sort Order" class="md:col-span-2 lg:col-span-2">
                <UInput
                  ...
                  size="lg"
                  class="w-full"
                  @update:model-value="(v: string | number) => { entry.draft.sortOrder = typeof v === 'number' ? v : parseSortOrder(v) }"
                />
              </UFormField>
              ...
              <UFormField label="Image" class="md:col-span-12">
                <div class="flex items-center gap-4">
                  <div v-if="entry.draft.imageUrl" class="w-20 h-20 rounded-lg overflow-hidden shrink-0 border border-pizza-border shadow-sm bg-elevated flex items-center justify-center">
                    <img :src="entry.draft.imageUrl" class="w-full h-full object-cover" />
                  </div>
                  <div v-else class="w-20 h-20 rounded-lg shrink-0 border border-dashed border-pizza-border bg-elevated flex items-center justify-center text-pizza-muted">
                    <UIcon name="i-lucide-image" class="size-6" />
                  </div>
                  <div class="flex items-center gap-2">
                    <UButton color="neutral" variant="soft" icon="i-lucide-upload" @click="clickFileInputRef(entry.item.id)">
                      {{ entry.draft.imageUrl ? 'Change Image' : 'Upload Image' }}
                    </UButton>
                    <!-- Hidden file input for logic -->
                    <!-- eslint-disable-next-line atx/no-native-input -->
                    <input
                      :ref="(el) => setFileRef(entry.item.id, el as HTMLInputElement | HTMLInputElement[] | null)"
                      type="file"
                      class="hidden"
                      accept="image/*"
                      @change="e => onUploadImage(e, entry.draft as unknown as ItemDraftWithImage)"
                    />
            ...
            <div class="pt-2">
              <div class="flex items-center justify-between mb-6 pb-2 border-b border-pizza-border">
                <h3 class="font-display text-2xl font-bold text-pizza-text tracking-tight">Pricing</h3>
                <p class="text-[11px] text-pizza-muted font-mono tracking-wider uppercase bg-elevated px-2 py-1 rounded-md">ID {{ entry.item.id }} · {{ entry.item.updatedAt }}</p>
              </div>
        ...
        <!-- Add new item -->
        <div class="rounded-2xl border-2 border-dashed border-primary-200 bg-primary-50/30 p-6 sm:p-8 shadow-sm">
          <div class="flex flex-col mb-8 border-b border-primary-100 pb-4">
            <h3 class="font-display text-2xl font-bold tracking-tight flex items-center gap-2 text-primary-900">
              <UIcon name="i-lucide-plus-circle" class="size-6 text-primary-600" />
              Add Menu Item
            </h3>
            <p class="text-primary-700/80 text-sm mt-2">Create a new item in the current category.</p>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-12 gap-6 items-start mt-6">
            ...
            <UFormField label="Sort Order" class="md:col-span-2">
              <UInput
                ...
                size="lg"
                class="w-full"
                @update:model-value="(v: string | number) => { newItem.sortOrder = typeof v === 'number' ? v : parseSortOrder(v) }"
              />
            </UFormField>
            ...
            <UFormField label="Image" class="md:col-span-12">
              <div class="flex items-center gap-4">
                <div v-if="newItem.imageUrl" class="w-20 h-20 rounded-lg overflow-hidden shrink-0 border border-primary-200 shadow-sm bg-elevated flex items-center justify-center">
                  <img :src="newItem.imageUrl" class="w-full h-full object-cover" />
                </div>
                <div v-else class="w-20 h-20 rounded-lg shrink-0 border border-dashed border-primary-200 bg-elevated flex items-center justify-center text-pizza-muted">
                  <UIcon name="i-lucide-image" class="size-6" />
                </div>
                <div class="flex items-center gap-2">
                  <UButton color="neutral" variant="soft" icon="i-lucide-upload" @click="() => ($refs.newFile as HTMLInputElement | undefined)?.click?.()">
                    {{ newItem.imageUrl ? 'Change Image' : 'Upload Image' }}
                  </UButton>
                  <!-- Hidden file input for logic -->
                  <!-- eslint-disable-next-line atx/no-native-input -->
                  <input ref="newFile" type="file" accept="image/*" class="hidden" @change="e => onUploadImage(e, newItem)" />
          ...
          <div class="mt-10">
            <div class="flex items-center justify-between mb-6 border-b border-primary-100 pb-2">
              <h4 class="font-display text-2xl font-bold text-primary-900 tracking-tight">Pricing</h4>
            </div>
            <AdminPriceEditor
